...
type Answer = 'Yes' | 'No'
export const EyeTestComponent : React.FC<TestComponentProps> = ({randomWords, allWords, restart, setCurrentTestIndex, currentTestIndex, onNextTest, onBackToList, onSubmitResults}) => {
    const { t } = useLanguage();
    const timer = 3000;
    ...
    useEffect(() => {
        ...
        if (currentIndex >= allWords.length) {
            ...
            if (onSubmitResults) {
                ...
                onSubmitResults(userAnswers, result, allWords.length);
            }
            setShowResult(true);
            return;
        }
                        ...
                        <div className="relative bg-[#E0E4E9] w-[178px] h-[178px] rounded-full p-4">
                            ...
                            <div className="bg-[#F2F5F9] w-full rounded-full h-full flex items-center justify-center">
                                <div className="flex flex-col gap-[4px] items-center">
                                    <span className="font-[500] text-[16px] leading-[16px]">{result/randomWords.length > 0.75 ? t('game.high_level') : result/randomWords.length>0.5 ? t('game.medium_level') : t('game.low_level')}</span>
                                    <div>
    ...
    return (
        <div className="flex flex-col items-center justify-center h-full">
            <div className="text-[20px] font-[600] text-[#1E1E1E] mb-4">
                {t('game.card_number')} {currentIndex + 1} {t('game.of')} {allWords.length}
            </div>
